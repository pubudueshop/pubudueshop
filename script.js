...
// Opens a product by ID, retries up to 3s if Firebase hasn't loaded yet.
// Must be on window so similar-product onclick cards can call it safely.
window.openProductWhenReady = function(id, attempt) {
    attempt = attempt || 0;
    var found = products.find(function(p) { return String(p.id) === String(id); });
    if (found) {
        openProductDetails(found.id);
        var pdv = document.getElementById('product-details-view');
        if (pdv) pdv.scrollTo({ top: 0, behavior: 'smooth' });
    } else if (attempt < 15) {
        setTimeout(function() { window.openProductWhenReady(id, attempt + 1); }, 200);
    } else {
        console.warn('openProductWhenReady: product not found after 3s, id=', id);
    }
};
